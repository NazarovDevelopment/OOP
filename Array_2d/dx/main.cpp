//--------------------------------------------------------------------------------------
// Óðîê 2. Ðèñîâàíèå òðåóãîëüíèêà. Îñíîâàí íà ïðèìåðå èç DX SDK (c) Microsoft Corp.
//--------------------------------------------------------------------------------------
#include <windows.h>
#include <d3d11.h>
#include <d3dx11.h>
#include <d3dcompiler.h>	// Äîáàâèëèñü íîâûå çàãîëîâêè
#include <xnamath.h>
#include "Resourse.h"
...
ID3D11DeviceContext*    g_pImmediateContext = NULL;	// Êîíòåêñò óñòðîéñòâà (ðèñîâàíèå)
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_hWnd = CreateWindow(L"Urok2WindowClass", L"Óðîê 2: Ðèñîâàíèå òðåóãîëüíèêà",
		WS_OVERLAPPEDWINDOW,
...
HRESULT InitGeometry()
{
	...
	// Âåðøèíû ìîãóò èìåòü ðàçëè÷íûå ïàðàìåòðû - êîîðäèíàòû â ïðîñòðàíñòâå, íîðìàëü, öâåò, êîîðäèíàòû
	// òåêñòóðû. Øàáëîí âåðøèí óêàçûâàåò, êàêèå èìåííî ïàðàìåòðû ñîäåðæàò âåðøèíû, êîòîðûå ìû ñîáèðàåìñÿ
	// èñïîëüçîâàòü. Íàøè âåðøèíû (SimpleVertex) ñîäåðæàò òîëüêî èíôîðìàöèþ î êîîðäèíàòàõ â ïðîñòðàíñòâå.
	// Çäåñü æå ìû óêàçûâàåì âåðøèííûé øåéäåð, êîòîðûé áóäåò èñïîëüçîâàòüñÿ äëÿ îáðàáîòêè èíôîðìàöèè î
	// íàøèõ âåðøèíàõ.
	D3D11_INPUT_ELEMENT_DESC layout[] =
	{
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		/* ñåìàíòè÷åñêîå èìÿ, ñåìàíòè÷åñêèé èíäåêñ, ðàçìåð, âõîäÿùèé ñëîò (0-15), àäðåñ íà÷àëà äàííûõ
		â áóôåðå âåðøèí, êëàññ âõîäÿùåãî ñëîòà (íå âàæíî), InstanceDataStepRate (íå âàæíî) */
	};
	...
	// Ñîçäàíèå áóôåðà âåðøèí (òðè âåðøèíû òðåóãîëüíèêà)
	...
	bd.ByteWidth = sizeof(SimpleVertex)* 3;	// ðàçìåð áóôåðà
	...
	InitData.pSysMem = vertices;				// óêàçàòåëü íà íàøè 3 âåðøèíû
	...
	// ò. å. òî÷êè 1-3 - ïåðâûé òðåóãîëüíèê, 4-6 - âòîðîé è ò. ä. Äðóãîé ñïîñîá - TRIANGLE STRIP.
	// Â ýòîì ñëó÷àå òî÷êè 1-3 - ïåðâûé òðåóãîëüíèê, 2-4 - âòîðîé, 3-5 - òðåòèé è ò. ä.
	// Â ýòîì ïðèìåðå åñòü òîëüêî îäèí òðåóãîëüíèê, ïîýòîìó ñïîñîá îòðèñîâêè íå èìååò çíà÷åíèÿ.
...
void Render()
{
	...
	float ClearColor[4] = { 0.0f, 0.0f, 1.0f, 1.0f }; // êðàñíûé, çåëåíûé, ñèíèé, àëüôà-êàíàë
	g_pImmediateContext->ClearRenderTargetView(g_pRenderTargetView, ClearColor);

	// Ïîäêëþ÷èòü ê óñòðîéñòâó ðèñîâàíèÿ øåéäåðû
	g_pImmediateContext->VSSetShader(g_pVertexShader, NULL, 0);
	g_pImmediateContext->PSSetShader(g_pPixelShader, NULL, 0);
	// Íàðèñîâàòü òðè âåðøèíû
